...
class CornersProblem(SearchProblem):
    def __init__(self, StareInitialaJoc):
        self.walls = StareInitialaJoc.getWalls()
        self.PozitieDeStart = StareInitialaJoc.getPacmanPosition()
        self.colturi = ( #colturile sunt in fct de dimensiunea hartii de joc, cu peretii exteriori exclusi
            (1, 1),
            (1, self.walls.height - 2),
            (self.walls.width - 2, 1),
            (self.walls.width - 2, self.walls.height - 2)
        )
        self._expanded = 0 # indexul pentru numarul de stari
        self.StareInitiala = (self.PozitieDeStart, (False, False, False, False))

    def getStartState(self):
        return self.StareInitiala

    def isGoalState(self, state):
        return all(state[1]) #aici verificam daca s-au vizitat toate cele 4 colturi

    def getSuccessors(self, state):
        successors = []
        PozitieCurenta, ColturiVizitate = state

        for directie in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x, y = PozitieCurenta
            dx, dy = Actions.directionToVector(directie)
            UrmatoareaPozitie = (int(x + dx), int(y + dy))
            if not self.walls[UrmatoareaPozitie[0]][UrmatoareaPozitie[1]]:
                ColturiVizitateNOI = list(ColturiVizitate)
                if UrmatoareaPozitie in self.colturi:
                    NrColt = self.colturi.index(UrmatoareaPozitie)
                    ColturiVizitateNOI[NrColt] = True
                successors.append(((UrmatoareaPozitie, tuple(ColturiVizitateNOI)), directie, 1))#aici adaugam succesorul, adica noua pozitie si starea colturilor, directia si costul=1
        self._expanded += 1 #crestem numarul de stari cu 1
        return successors

    def getCostOfActions(self, Actiuni):
        if Actiuni is None:
            return 999999 #solutie invalida
        x, y = self.PozitieDeStart
        for actiune in Actiuni:
            dx, dy = Actions.directionToVector(Actiuni)
